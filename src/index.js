...
import { fetchBookmarks } from './bird.js';
import { loadState, saveState, findNewBookmarks } from './state.js';
...
async function main() {
  console.log('');
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║                 🧠 Synapse - Bookmark Sync                 ║');
  console.log('╚════════════════════════════════════════════════════════════╝');
  console.log('');
  console.log(`📅 Started at: ${new Date().toLocaleString()}`);
  console.log(`📁 Vault: ${config.vaultPath}`);
  console.log(`📂 Folder: ${config.bookmarksFolder}`);
  
  if (isFullSync) {
    console.log('');
    console.log('🔄 FULL SYNC MODE - Fetching ALL bookmarks (ignoring maxPages limit)');
  }
  ...
  try {
    // ─────────────────────────────────────────────────────────────
    // STEP 1: Fetch bookmarks from Twitter
    // ─────────────────────────────────────────────────────────────
    console.log('');
    console.log('📥 STEP 1: Fetching bookmarks from Twitter');
    console.log('─'.repeat(60));
    
    const bookmarks = await fetchBookmarks({ fullSync: isFullSync, dryRun: isDryRun });
    ...
    // ─────────────────────────────────────────────────────────────
    // STEP 2: Load state and find new bookmarks
    // ─────────────────────────────────────────────────────────────
    console.log('');
    console.log('🔄 STEP 2: Checking for new bookmarks');
    console.log('─'.repeat(60));
    
    const processedIds = loadState();
    const newBookmarks = findNewBookmarks(bookmarks, processedIds);
    
    if (newBookmarks.length === 0) {
      console.log('');
      console.log('✨ All bookmarks already processed!');
      console.log('   Nothing new to sync. Exiting.');
      return;
    }

    // Apply limit if configured
    let toProcess = newBookmarks;
    if (config.maxTweetsPerRun > 0 && newBookmarks.length > config.maxTweetsPerRun) {
      console.log('');
      console.log(`⚡ Rate limiting: Processing ${config.maxTweetsPerRun} of ${newBookmarks.length} new bookmarks`);
      toProcess = newBookmarks.slice(0, config.maxTweetsPerRun);
    }
    ...
    // Get existing categories (will be updated as we create new ones)
    ...
    for (let batchIdx = 0; batchIdx < totalBatches; batchIdx++) {
      ...
      // 4b. Write this batch to Obsidian immediately
      console.log('');
      console.log('      💾 Writing to Obsidian...');
      const results = saveTweetsToVault(batch, batchCategories, isDryRun);
    ...
    console.log('');
    console.log('📈 Summary:');
    console.log(`   • Total bookmarks fetched: ${bookmarks.length}`);
    console.log(`   • New bookmarks processed: ${totalProcessed}`);
    console.log(`   • Batches processed: ${totalBatches}`);
    console.log(`   • Categories used: ${categoryCount.size}`);
    console.log(`   • Total time: ${totalTime}s`);
    console.log(`   • Vault location: ${config.vaultPath}/${config.bookmarksFolder}`);
    console.log('');
